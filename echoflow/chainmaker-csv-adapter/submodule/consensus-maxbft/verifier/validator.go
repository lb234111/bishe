...
func NewValidator(validators []string, verifier protocol.BlockVerifier,
	...
	epochId, epochEndView uint64, logger protocol.Logger, conf *config.ChainConfig) Verifier {
...
func (v *Validator) ValidateProposal(proposal *maxbftpb.ProposalData, qc *maxbftpb.QuorumCert,
	...
	v.logger.Debugf("ValidateProposal success [%d:%d:%x], chainConfig after verify: +%v",
		header.BlockHeight, proposal.View, header.BlockHash, v.config)
	return result, nil
}
...
func (v *Validator) checkContractTxRwSet(proposal *maxbft.ProposalData) error {
	...
	if txRwSet, err = utils.ConstructContractTxRwSet(proposal.View,
		v.epochEndView, v.epochId, v.config, v.store, v.logger); err != nil {
