...
func NewPaceMaker(curView uint64, fork forest.Forester, roundTime, roundTimeInterval,
	...
	log.Debugf("update param-1 timeout NewPaceMaker(), RoundTimeout: %v,"+
		" RoundTimeoutInterval: %v, MaxTimeout: %v",
		pacemaker.roundTimeout, pacemaker.roundTimeoutInterval, pacemaker.maxTimeout)
...
func (pm *DefaultPaceMaker) OnTimeout(event *timeservice.TimerEvent) *timeservice.TimerEvent {
	...
	pm.logger.Debugf("checkpoint OnTimeout(), RoundTimeout: %v,", roundTimeout)
...
func (pm *DefaultPaceMaker) AddEvent(eventType maxbftpb.ConsStateType) *timeservice.TimerEvent {
	roundTimeout := pm.getEventTimeout()
	pm.logger.Debugf("checkpoint AddEvent(), RoundTimeout: %v,", roundTimeout)
	event := &timeservice.TimerEvent{
		View:     pm.curView,
		Duration: roundTimeout,
		Type:     eventType,
...
func (pm *DefaultPaceMaker) getEventTimeout() time.Duration {
	...
	pm.logger.Debugf("checkpoint0 getEventTimeout(), pm.roundTimeout: %v, timeout: %v,", pm.roundTimeout, timeout)
