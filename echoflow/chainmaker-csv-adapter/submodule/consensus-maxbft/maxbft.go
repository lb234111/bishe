...
var (
	...
	//try to change consistentMsgCapacity while running blockchain, failed
	//DefaultConsistentMsgCapacity int64 = 1024
	//ConsistentMsgCapacityMill = "MaxBFTConsistentMsgCapacityMill"
)
...
func New(cfg *consensusUtils.ConsensusImplConfig) (protocol.ConsensusEngine, error) {
	...
	//try to change consistentMsgCapacity while running blockchain, failed
	//var consistentMsgCapacity = GetConsistentMsgCapacityFromChainConf(cfg)
	//log.Debugf("update param-3 ConsistentMsgCapacity %d", consistentMsgCapacity)
...
// Try to get ConsistentMsgCapacity configuration from the given chain configurations,  failed
//Because ConsistentMsgCapacity means the size of chan, the size may be fixed when the chan is made
//func GetConsistentMsgCapacityFromChainConf(cfg *consensusUtils.ConsensusImplConfig) int64 {
//	conConf := cfg.ChainConf.ChainConfig().Consensus.ExtConfig
//	for _, oneConf := range conConf {
//		switch oneConf.Key {
//		case ConsistentMsgCapacityMill:
//			if v, err := strconv.ParseInt(oneConf.Value, 10, 64); err == nil {
//				return v
//			}
//		}
//	}
//	return DefaultConsistentMsgCapacity
//}
