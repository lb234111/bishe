...
var (
	...
	TBFT_provote_delta_timeout_key  = "TBFT_prevote_delta_timeout"
	TBFT_prevote_timeout_key        = "TBFT_prevote_timeout"
	TBFT_precommit_timeout_key      = "TBFT_precommit_timeout"
	TBFT_prcommit_delta_timeout_key = "TBFT_precommit_delta_timeout"
)
...
const (
	...
	DefaultTimeoutPrevote        = 30 * time.Second // Timeout of waitting for >2/3 prevote
	DefaultTimeoutPrevoteDelta   = 1 * time.Second  // Increased time delta of TimeoutPrevote between round
	DefaultTimeoutPrecommit      = 30 * time.Second // Timeout of waitting for >2/3 precommit
	DefaultTimeoutPrecommitDelta = 1 * time.Second  // Increased time delta of TimeoutPrecommit between round
...
func (consensus *ConsensusTBFTImpl) Start() error {
	...
	consensus.msgbus.Register(msgbus.RecvConsistentMsg, consensus)
...
func (consensus *ConsensusTBFTImpl) OnMessage(message *msgbus.Message) {
	...
	case msgbus.RecvConsensusMsg:
		if msg, ok := message.Payload.(*netpb.NetMsg); ok {
			if consensusMsg := createConsensusMsgFromTBFTMsgBz(msg.Payload); consensusMsg != nil {
...
func (consensus *ConsensusTBFTImpl) Verify(consensusType consensuspb.ConsensusType,
	...
	_, _, _, _, _, _, _, _, _, err := consensus.extractConsensusConfig(consensusConfig)
	return err
}
...
func (consensus *ConsensusTBFTImpl) updateChainConfig() (addedValidators []string, removedValidators []string,
	...
	validators, timeoutPropose, timeoutProposeDelta, timeoutProvote, timeoutPrevoteDelta, timeoutPrecommit, timeoutPrecommitDelta, timeoutCommit, tbftBlocksPerProposer,
		err := consensus.extractConsensusConfig(config)
	if err != nil {
		return nil, nil, err
	}
	consensus.logger.Infof("[%s](%d/%d/%v) extract chain config, config: %v,"+
		" TimeoutPropose: %v, TimeoutProposeDelta: %v,TimeoutPrevote :%v,TimePrevoteDelta :%v,TimePrecommit :%v,TimePrecommitDelta :%v,TimeCommit :%v,BlocksPerProposer %v err: %v",
		consensus.Id, consensus.Height, consensus.Round, consensus.Step, config,
		timeoutPropose, timeoutProposeDelta, timeoutProvote, timeoutPrevoteDelta, timeoutPrecommit,
		timeoutPrecommitDelta, timeoutCommit, tbftBlocksPerProposer, err)

	consensus.TimeoutPropose = timeoutPropose
	consensus.TimeoutProposeDelta = timeoutProposeDelta
	consensus.TimeoutPrevote = timeoutProvote //
	consensus.TimeoutPrevoteDelta = timeoutPrevoteDelta
	consensus.TimeoutPrecommit = timeoutPrecommit
	consensus.TimeoutPrecommitDelta = timeoutPrecommitDelta
	consensus.logger.Infof("[%s](%d/%d/%v) updated chain config, config: %v,"+
		" TimeoutPropose: %v, TimeoutProposeDelta: %v,TimeoutPrevote :%v,TimePrevoteDelta :%v,TimePrecommit :%v,TimePrecommitDelta :%v,TimeCommit :%v, BlocksPerProposer: %v",
		consensus.Id, consensus.Height, consensus.Round, consensus.Step, config,
		consensus.TimeoutPropose, consensus.TimeoutProposeDelta, consensus.TimeoutPrevote, consensus.TimeoutPrevoteDelta, consensus.TimeoutPrecommit,
		consensus.TimeoutPrecommitDelta, timeoutCommit, tbftBlocksPerProposer)
...
func (consensus *ConsensusTBFTImpl) extractConsensusConfig(config *config.ConsensusConfig) (validators []string,
	timeoutPropose time.Duration, timeoutProposeDelta time.Duration, timeoutPrevote time.Duration,
	timeoutProvoteDelta time.Duration, timeoutPrecommit time.Duration, timeoutPrecommitDelta time.Duration,
	timeoutCommit time.Duration, tbftBlocksPerProposer uint64, err error) {
	timeoutPropose = DefaultTimeoutPropose
	timeoutProposeDelta = DefaultTimeoutProposeDelta
	tbftBlocksPerProposer = uint64(1)
	timeoutPrevote = DefaultTimeoutPrevote
	timeoutProvoteDelta = DefaultTimeoutPrevoteDelta
	timeoutPrecommit = DefaultTimeoutPrecommit
	timeoutPrecommitDelta = DefaultTimeoutPrecommitDelta
	...
	for _, v := range config.ExtConfig {
		...
		case TBFT_prevote_timeout_key:
			timeoutPrevote, err = consensus.extractPrevoteTime(v.Value)
		case TBFT_provote_delta_timeout_key:
			timeoutProvoteDelta, err = consensus.extractPrevoteTimeoutDelta(v.Value)
		case TBFT_precommit_timeout_key:
			timeoutPrecommit, err = consensus.extractPrecommitTimeout(v.Value)
		case TBFT_prcommit_delta_timeout_key:
			timeoutPrecommitDelta, err = consensus.extractPrecommitTimeoutDelta(v.Value)
		}
...
func (consensus *ConsensusTBFTImpl) extractPrecommitTimeout(value string) (timeoutPrecommit time.Duration, err error) {
	if timeoutPrecommit, err = time.ParseDuration(value); err != nil {
		consensus.logger.Infof("[%s](%d/%v) update timeoutPrecommit config, TimeoutPrecommit: %v,"+
			" TimeoutPrecommitDelta: %v"+"parse timeoutPrecommit error: %v",
			consensus.Id, consensus.Height, consensus.Round, consensus.Step,
			timeoutPrecommit, consensus.TimeoutPrecommitDelta, err)
	}
	return
}

func (consensus *ConsensusTBFTImpl) extractPrecommitTimeoutDelta(value string) (timeoutPrecommitDelta time.Duration, err error) {
	if timeoutPrecommitDelta, err = time.ParseDuration(value); err != nil {
		consensus.logger.Infof("[%s](%d/%v) update timeoutPrecommitDelta config, TimeoutPrecommit: %v,"+
			" TimeoutPrecommitDelta: %v"+"parse TimeoutPrecommitDelta error: %v",
			consensus.Id, consensus.Height, consensus.Round, consensus.Step,
			consensus.TimeoutPrecommit, timeoutPrecommitDelta, err)
	}
	return
}

func (consensus *ConsensusTBFTImpl) extractPrevoteTime(value string) (timeoutPrevote time.Duration, err error) {
	if timeoutPrevote, err = time.ParseDuration(value); err != nil {
		consensus.logger.Infof("[%s](%d/%v) update timeoutPrevote config, TimeoutPrevote: %v,"+
			" TimeoutPrevoteDelta: %v"+"parse TimeoutPrevote error: %v",
			consensus.Id, consensus.Height, consensus.Round, consensus.Step,
			timeoutPrevote, consensus.TimeoutPrevoteDelta, err)
	}
	return
}

func (consensus *ConsensusTBFTImpl) extractPrevoteTimeoutDelta(value string) (timeoutPrevoteDelta time.Duration, err error) {
	if timeoutPrevoteDelta, err = time.ParseDuration(value); err != nil {
		consensus.logger.Infof("[%s](%d/%v) update timeoutPrevote_delta config, TimeoutPrevote: %v,"+
			" timeoutPrevoteDelta: %v"+"parse timeoutPrevoteDelta error: %v",
			consensus.Id, consensus.Height, consensus.Round, consensus.Step,
			consensus.TimeoutPrevote, timeoutPrevoteDelta, err)
	}
...
func (consensus *ConsensusTBFTImpl) procPropose(proposal *tbftpb.Proposal) {
	...
	// Failed to validate proposal, do not continue processing
	...
	verifyProposalTime := utils.CurrentTimeMillisSeconds()
	...
	consensus.logger.Infof("[%s](%d/%d/%s) processed proposal (%d/%d/%x), time[verify:%dms]",
		consensus.Id, consensus.Height, consensus.Round, consensus.Step,
		proposal.Height, proposal.Round, proposal.Block.Header.BlockHash,
		verifyProposalTime-receiveProposalTime)
}
...
func (consensus *ConsensusTBFTImpl) PrevoteTimeout(round int32) time.Duration {
	return time.Duration(
		consensus.TimeoutPrevote.Nanoseconds()+consensus.TimeoutPrevoteDelta.Nanoseconds()*int64(round),
	) * time.Nanosecond
}
...
func (consensus *ConsensusTBFTImpl) PrecommitTimeout(round int32) time.Duration {
	return time.Duration(
		consensus.TimeoutPrecommit.Nanoseconds()+consensus.TimeoutPrecommitDelta.Nanoseconds()*int64(round),
	) * time.Nanosecond
}
...
func (consensus *ConsensusTBFTImpl) CommitTimeout(round int32) time.Duration {
	return time.Duration(DefaultTimeoutCommit.Nanoseconds()*int64(round)) * time.Nanosecond
}
	...
	if consensus.Proposal != nil {
		...
		if !isNilHash(hash) && (consensus.ValidRound < vote.Round) && (vote.Round == consensus.Round) {
			if bytes.Equal(hash, consensus.Proposal.PbMsg.Block.Header.BlockHash) {
				...
				consensus.logger.Debugf("proposal [%x] is not match majority[%x], need new round, set ProposalBlock=nil",
...
func (consensus *ConsensusTBFTImpl) enterPropose(height uint64, round int32) {
	...
	// Calculate timeout
	validator, _ := consensus.validatorSet.GetProposer(height, round)
	consensus.validatorSet.Lock()
	...
	if validatorHeight != 0 {
		t := consensus.Height - validatorHeight
		if consensus.Height <= validatorHeight || validatorHeight == 0 {
			t = 1
		}
...
func (consensus *ConsensusTBFTImpl) getFutureMsgFromCache(height uint64, round int32) {
	...
	if roundVs != nil {
		...
		for _, precommit := range roundVs.Prevotes.Votes {
			consensus.externalMsgC <- &ConsensusMsg{
...
// @return *ConsensusMsg
//
func createConsensusMsgFromTBFTMsgBz(tbftMsgBz []byte) *ConsensusMsg {
