...
func (pool *normalPool) Start() (err error) {
	...
	if !atomic.CompareAndSwapInt32(&pool.stat, 0, 1) {
		pool.log.Errorf(commonErr.ErrTxPoolHasStarted.String())
		return commonErr.ErrTxPoolHasStarted
	}
	// msgBus should not be nil
	if pool.msgBus != nil {
		pool.msgBus.Register(msgbus.RecvTxPoolMsg, pool)
	} else {
		panic("should not be happen, msgBus in normal pool should not be nil")
	}
	// replay txs
...
func (pool *normalPool) OnMessage(msg *msgbus.Message) {
	...
	if atomic.LoadInt32(&pool.stat) == 0 {
		return
	}
	// should not be nil
	if msg == nil {
	...
	if err := proto.Unmarshal(txPoolBz, txPoolMsg); err != nil {
		pool.log.Errorf("OnMessage unmarshal txPoolMsg failed, err:%v", err)
		return

	}
	pool.log.Infof("txtype:%v", txPoolMsg.Type)
	switch txPoolMsg.Type {
	case txpoolPb.TxPoolMsgType_SINGLE_TX:
		// unMarshal to tx
		tx := &commonPb.Transaction{}
		if err := proto.Unmarshal(txPoolMsg.Payload, tx); err != nil {
			pool.log.Errorf("OnMessage unmarshal tx failed, err:%v", err)
			return
		}
	...
	case txpoolPb.TxPoolMsgType_BATCH_TX:
		...
		if err := proto.Unmarshal(txPoolMsg.Payload, txBatch); err != nil {
			pool.log.Errorf("OnMessage unmarshal txBatch failed, err:%v", err)
			return
		}
		...
		// recorder
		// TransactionThroughputs := make([]*recorder.TransactionThroughput, 0)
		strs := ""
		for _, tx := range txs {
	...
	case txpoolPb.TxPoolMsgType_RECOVER_REQ:
		...
		if err := proto.Unmarshal(txPoolMsg.Payload, txRecoverReq); err != nil {
			pool.log.Errorf("OnMessage unmarshal txRecoverRequest failed, err:%v", err)
			return
		}
	...
	case txpoolPb.TxPoolMsgType_RECOVER_RESP:
		...
		if err := proto.Unmarshal(txPoolMsg.Payload, txRecoverRes); err != nil {
			pool.log.Errorf("OnMessage unmarshal txRecoverResponse failed, err:%v", err)
			return
		}
	...
	default:
		pool.log.Errorf("OnMessage receive invalid message type %s", txPoolMsg.Type)
	}
}
...
func (pool *normalPool) AddTx(tx *commonPb.Transaction, source protocol.TxSource) error {
	...
	// 		TxID_satran:      tx.Payload.GetTxId(),
	// 		OccurTime_satran: time.Now(),
	// 		Source_satran:    int(source),
	// 	}
	// 	performance.Record(transaction, nil)
	// }
	//pool.log.Infof("Add a tx, txId:%s, source:%v", tx.Payload.GetTxId(), source)
	//recorder
	// File, err := os.OpenFile("/home/yaoye/projects/chainmaker-csv/temp/chainmaker-csv/log/TransactionThroughput.csv", os.O_RDWR|os.O_APPEND|os.O_CREATE, 0666)
	// if err != nil {
	// 	pool.log.Errorf("File open failed!")
	...
	// 	if err != nil {
	// 		// 记录数据出现错误
	// 		pool.log.Errorf("[TransactionThroughput] record failed:submodule/txpool-normal/v2@v2.3.0/normal_tx_pool.go:AddTx()")
	// 	} else {
	// 		// 记录数据成功
	// 		pool.log.Infof("[TransactionThroughput] record succeed:submodule/txpool-normal/v2@v2.3.0/normal_tx_pool.go:AddTx()")
	// 	}
	// case <-time.NewTimer(time.Second).C:
	// 	// 记录数据超时
	// 	pool.log.Errorf("[TransactionThroughput] record timeout:submodule/txpool-normal/v2@v2.3.0/normal_tx_pool.go:AddTx()")
	...
	// 	err = <-signal
	// 	if err != nil {
	// 		pool.log.Infof("mysql write error")
	// 	}
	// }
...
func (pool *normalPool) Stop() error {
	// should not be stopped again
	if !atomic.CompareAndSwapInt32(&pool.stat, 1, 0) {
		pool.log.Errorf(commonErr.ErrTxPoolHasStopped.String())
		return commonErr.ErrTxPoolHasStopped
	}
	// dump txs to wal
	if err := pool.dumpTxs(); err != nil {
		// even if dump txs failed, it should stop tx_pool service
		pool.log.Errorf("dump txs failed, err:%v", err)
	}
	// cancel context
	pool.cancel()
	// close addTxCh channel
	close(pool.addTxCh)
	// close buildBatchCh channel
	pool.batchBuilder.Stop()
