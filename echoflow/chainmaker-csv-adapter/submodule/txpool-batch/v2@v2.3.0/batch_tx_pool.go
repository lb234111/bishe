...
func (pool *batchTxPool) Start() (err error) {
	...
	if pool.msgBus != nil {
		...
		panic("should not be happen, msgBus in batch pool should not be nil")
	}
	// replay tx batches in wal
	if err = pool.replayTxs(); err != nil {
		atomic.StoreInt32(&pool.stat, 0)
		pool.log.Errorf("replay dumped tx batches failed, err:%v", err)
		return err
	}
...
func (pool *batchTxPool) OnMessage(msg *msgbus.Message) { // nolint
	...
	case txpoolPb.TxPoolMsgType_SINGLE_TX:
		...
		if err := proto.Unmarshal(txPoolMsg.Payload, tx); err != nil {
			pool.log.Errorf("OnMessage unmarshal transaction failed, err:%v", err)
			return
		}
		// add tx to pool
	...
	case txpoolPb.TxPoolMsgType_RECOVER_REQ:
		...
		if err := proto.Unmarshal(txPoolMsg.Payload, batchReq); err != nil {
			pool.log.Errorf("OnMessage unmarshal txBatchRecoverRequest failed, err:%v", err)
			return
		}
	...
	case txpoolPb.TxPoolMsgType_RECOVER_RESP:
		...
		if err := proto.Unmarshal(txPoolMsg.Payload, batchRes); err != nil {
			pool.log.Errorf("OnMessage unmarshal txBatchRecoverResponse failed, err:%v", err)
			return
		}
...
func (pool *batchTxPool) FetchTxBatches(blockHeight uint64) (batchIds []string, txsTable [][]*commonPb.Transaction) {
	...
	mBatches := pool.queue.fetchTxBatches()
	if len(mBatches) == 0 {
		return
	}
	// prune txs
...
func (pool *batchTxPool) Stop() error {
	...
	if !atomic.CompareAndSwapInt32(&pool.stat, 1, 0) {
		pool.log.Errorf(commonErrors.ErrTxPoolHasStopped.String())
		return commonErrors.ErrTxPoolHasStopped
	}
	// dump tx batches
