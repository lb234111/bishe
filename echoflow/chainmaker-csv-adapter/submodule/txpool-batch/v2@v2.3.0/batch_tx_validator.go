...
func (pool *batchTxPool) validateTxBatch(batch *txpoolPb.TxBatch, batchSource TxBatchSourceMod) (
	...
	// set txSource
	var txSource = protocol.P2P
	if batchSource == TxBatchByRetry {
		txSource = protocol.INTERNAL
	}
	// verify whether txs in batch are valid concurrently
	dbHeight, filter := pool.validateTxs(batch.Txs, txSource)
	// create memTxBatch
...
func (pool *batchTxPool) retryTxBatchValidate(mBatch *memTxBatch) error {
	// verify whether txs in batch are valid concurrently
	dbHeight, filter := pool.validateTxs(mBatch.getTxs(), protocol.INTERNAL)
...
func (pool *batchTxPool) validateTxs(txs []*commonPb.Transaction, source protocol.TxSource) (
	uint64, []bool) {
	// should not be nil
	if len(txs) == 0 {
...
func (pool *batchTxPool) validateTx(tx *commonPb.Transaction, source protocol.TxSource) (*memTx, error) {
	// verify timestamp
	if err := validateTxTime(tx, pool.chainConf); err != nil {
		pool.log.Warnf("transaction timestamp is timeout, txId:%s", tx.Payload.GetTxId())
		return nil, err
	}
	// verify signature and format for the source is P2P
