...
func (chain *BlockCommitterImpl) AddBlock(block *commonPb.Block) (err error) {
	...
	// Txconfirmdelay_ends := make([]*recorder.Txconfirmdelay_end, 0)
	strs := ""
	for _, tx1 := range block.Txs {
		// writecsv
		// File, err := os.OpenFile("/home/yaoye/projects/chainmaker-csv/temp/chainmaker-csv/log/Txconfirmdelay_ends.csv", os.O_RDWR|os.O_APPEND|os.O_CREATE, 0666)
		// if err != nil {
		// 	chain.log.Errorf("File open failed!")
		// }
		// defer File.Close()
		//创建写入接口
		// WriterCsv := csv.NewWriter(File)
		str := fmt.Sprintf("%s,%s\n", time.Now().Format("2006-01-02 15:04:05.000000"), tx1.Payload.TxId) //需要写入csv的数据，切片类型
		strs = strs + str

		//......teststart by gzy
		// record Txconfirmdelay_end
		// txconfirmdelay_end := &recorder.Txconfirmdelay_end{
		// 	Tx_timestamp:          time.Now(),
		// 	Txid_confirmdelay_end: tx1.Payload.TxId,
		// 	Txconfirmtime:         time.Now(),
		// }
		// resultC := make(chan error, 1)
		// recorder.Record(txconfirmdelay_end, resultC)
		// recorder.RecordBatch(Txconfirmdelay_ends, Txconfirmdelay_ends[0], resultC)
		// select {
		// case err := <-resultC:
		// 	if err != nil {
		// 		// 记录数据出现错误
		// 		chain.log.Errorf("[txconfirmdelay_end] record failed:module/core/common/block_helper.go:AddBlock")
		// 		chain.log.Errorf("[txconfirmdelay_end] *****:%s", err.Error())
		// 	} else {
		// 		// 记录数据成功
		// 		chain.log.Infof("[txconfirmdelay_end] record succeed:module/core/common/block_helper.go:AddBlock")
		// 	}
		// case <-time.NewTimer(time.Second).C:
		// 	// 记录数据超时
		// 	chain.log.Errorf("[txconfirmdelay_end] record timeout:module/core/common/block_helper.go:AddBlock")
		// }
		// [Txconfirmdelay_ends = append(Txconfirmdelay_ends, &recorder.Txconfirmdelay_end{
		// 	Tx_timestamp:          time.Now(),
		// 	Txid_confirmdelay_end: tx1.Payload.TxId,
		// 	Txconfirmtime:         time.Now(),
		// })]
	...
	// if err != nil {
	// 	if err == errors.New("close") {
	// 		chain.log.Errorf("[tx_delay_end] switch closed")
	// 	} else {
	// 		chain.log.Errorf("[tx_delay_end] record failed")
	// 	}
	// }
	// chain.log.Infof("[tx_delay_end] record succeed")

	//......teststart by gzy
	// record Txtpsblockdelay_end
	// File, err := os.OpenFile("/home/yaoye/projects/chainmaker-csv/temp/chainmaker-csv/log/Txtpsblockdelay_end.csv", os.O_RDWR|os.O_APPEND|os.O_CREATE, 0666)
	// if err != nil {
	// 	chain.log.Errorf("File open failed!")
	// }
	// defer File.Close()
	...
	// if err != nil {
	// 	if err == errors.New("close") {
	// 		chain.log.Errorf("[block_commit_duration_end] switch closed")
	// 	} else {
	// 		chain.log.Errorf("[block_commit_duration_end] record failed")
	// 	}
	// }
	// chain.log.Infof("[block_commit_duration_end] record succeed")
	// txtpsblockdelay_end := &recorder.Txtpsblockdelay_end{
	// 	Tx_timestamp:             time.Now(),
	// 	Height_delay_end:         height,
	// 	Txs_tps_end:              lastProposed.Header.TxCount,
	// 	Txconfirmtime_blockdelay: time.Now(),
	// }
	// resultC := make(chan error, 1)
	// recorder.Record(txtpsblockdelay_end, resultC)
	// select {
	// case err := <-resultC:
	// 	if err != nil {
	// 		// 记录数据出现错误
	// 		chain.log.Errorf("[Txtpsblockdelay_end] record failed:module/core/common/block_helper.go:AddBlock()")
	// 	} else {
	// 		// 记录数据成功
	// 		chain.log.Infof("[Txtpsblockdelay_end] record succeed:module/core/common/block_helper.go:AddBlock()")
	// 	}
	// case <-time.NewTimer(time.Second).C:
	// 	// 记录数据超时
	// 	chain.log.Errorf("[Txtpsblockdelay_end] record timeout:module/core/common/block_helper.go:AddBlock()")
	// }

	// record SampleConflict
	// conflict_num := &recorder.SampleConflict{
	// 	Tx_timestamp:  time.Now(),
	// 	CoFL:          int(getConflict(block.Dag)),
	// 	OccurTime_con: time.Now(),
	// 	Blokheit:      int(height),
	// }

	// File, err = os.OpenFile("/home/yaoye/projects/chainmaker-csv/temp/chainmaker-csv/log/SampleConflict.csv", os.O_RDWR|os.O_APPEND|os.O_CREATE, 0666)
	// if err != nil {
	// 	chain.log.Errorf("File open failed!")
	// }
	// defer File.Close()
	...
	// if err != nil {
	// 	if err == errors.New("close") {
	// 		chain.log.Errorf("[block_tx_conflict_rate] switch closed")
	// 	} else {
	// 		chain.log.Errorf("[block_tx_conflict_rate] record failed")
	// 	}
	// }
	// chain.log.Infof("[block_tx_conflict_rate] record succeed")

	// conflict_num := &recorder.SampleConflict{
	// 	Tx_timestamp:  time.Now(),
	// 	CoFL:          int(getConflict(block.Dag)),
	// 	OccurTime_con: time.Now(),
	// 	TxCount:       int(lastProposed.Header.TxCount),
	// }
	// resultC2 := make(chan error, 1)
	// recorder.Record(conflict_num, resultC2) // 这里会启动一个 goroutine，异步执行写入数据库
	// select {
	// case err := <-resultC2:
	// 	if err != nil {
	// 		// 记录数据出现错误
	// 		chain.log.Errorf("[SampleConflict] record failed:module/core/common/block_helper.go:AddBlock()")
	// 	} else {
	// 		// 记录数据成功
	// 		chain.log.Infof("[SampleConflict] record succeed:module/core/common/block_helper.go:AddBlock()")
	// 	}
	// case <-time.NewTimer(time.Second).C:
	// 	// 记录数据超时
	// 	chain.log.Errorf("[SampleConflict] record timeout:module/core/common/block_helper.go:AddBlock()")
	// }
