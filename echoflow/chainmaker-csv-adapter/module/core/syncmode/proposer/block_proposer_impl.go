...
func (bp *BlockProposerImpl) proposing(height uint64, preHash []byte) *commonpb.Block {
	...
	// record txtpsblockdelay_start
	// File, err := os.OpenFile("/home/yaoye/projects/chainmaker-csv/temp/chainmaker-csv/log/Txtpsblockdelay_start.csv", os.O_RDWR|os.O_APPEND|os.O_CREATE, 0666)
	// if err != nil {
	// 	bp.log.Errorf("File open failed!")
	// }
	// defer File.Close()
	//创建写入接口
	...
	// 		bp.log.Errorf("[block_commit_duration_start] switch closed")
	// 	} else {
	// 		bp.log.Errorf("[block_commit_duration_start] record failed")
	// 	}
	// }
	// bp.log.Infof("[block_commit_duration_start] record succeed")
	// txtpsblockdelay_start := &recorder.Txtpsblockdelay_start{
	// 	Tx_timestamp:          time.Now(),
	// 	Height_delay_sta:      height,
	// 	Total_tps_start:       elapsed,
	// 	Txsendtime_blockdelay: time.Now(),
	// }
	// resultC := make(chan error, 1)
	// recorder.Record(txtpsblockdelay_start, resultC)
	// select {
	// case err := <-resultC:
	// 	if err != nil {
	// 		// 记录数据出现错误
	// 		bp.log.Errorf("[Txtpsblockdelay_start] record failed:module/core/syncmode/proposer/block_proposer_impl.go:proposing()")
	// 	} else {
	// 		// 记录数据成功
	// 		bp.log.Infof("[Txtpsblockdelay_start] record succeed:module/core/syncmode/proposer/block_proposer_impl.go:proposing()")
	// 	}
	// case <-time.NewTimer(time.Second).C:
	// 	// 记录数据超时
	// 	bp.log.Errorf("[Txtpsblockdelay_end] record timeout:module/core/syncmode/proposer/block_proposer_impl.go:proposing()")
	// }
